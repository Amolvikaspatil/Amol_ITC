...
select salesperson, avg(sales_amount) over() from sales;
...
SELECT salesperson, sale_date, sales_amount,
       LAG(sales_amount,3) OVER (PARTITION BY salesperson ORDER BY sale_date) AS prev_sale,
       LEAD(sales_amount,2) OVER (PARTITION BY salesperson ORDER BY sale_date desc) AS next_sale
FROM sales ;
...
SELECT salesperson, sale_date, sales_amount,
       AVG(sales_amount) OVER (PARTITION BY salesperson ORDER BY sale_date ROWS BETWEEN 2 PRECEDING AND CURRENT ROW ) AS moving_avg
FROM sales;
